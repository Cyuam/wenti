...
(function () {
    ...
    function renderFiles() {
        ...
        if (!data.length) {
            list.innerHTML = '<p class="empty-tip">暂无文件。请把文件放到仓库 downloads/ 目录，并在 files.js 中登记。</p>';
            return;
        }
        list.innerHTML = data.map(function (f) {
            return ''
                + '<a class="file-card" href="' + f.file + '" download>'
                + '  <div class="file-card-head">'
                + '    <span class="file-tag">' + escapeHtml(f.tag || '文件') + '</span>'
                + '    <span class="file-name">' + escapeHtml(f.name) + '</span>'
                + '  </div>'
                + '  <p class="file-desc">' + escapeHtml(f.desc || '') + '</p>'
                + '  <span class="file-btn">下载</span>'
                + '</a>';
        }).join('');
    }
